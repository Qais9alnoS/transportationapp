...
def create_route_path(route_path: RoutePathCreate, db: Session = Depends(get_db)):
    # تحقق من وجود route_id
    route = db.query(models.Route).filter(models.Route.id == route_path.route_id).first()
    if not route:
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail="خطأ في حفظ نقطة المسار: تأكد من صحة البيانات المدخلة وعدم تكرار النقطة")
