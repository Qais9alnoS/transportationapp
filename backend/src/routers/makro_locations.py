...
def create_makro_location(loc: MakroLocationCreate, db: Session = Depends(get_db)):
    ...
    """
    # تحقق من عدم تكرار makro_id في نفس الإحداثيات
    existing = db.query(models.MakroLocation).filter(
        models.MakroLocation.makro_id == loc.makro_id,
        models.MakroLocation.lat == loc.lat,
        models.MakroLocation.lng == loc.lng
    ).first()
    if existing:
        raise HTTPException(status_code=400, detail="سجل makro_id مع نفس الإحداثيات موجود مسبقًا")
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail="خطأ في حفظ موقع المكرو: تأكد من صحة البيانات المدخلة وعدم تكرار الموقع")
    return db_loc
...
def update_makro_location(makro_location_id: int, loc: MakroLocationCreate, db: Session = Depends(get_db)):
    ...
    db_loc.timestamp = loc.timestamp or datetime.now(timezone.utc)
