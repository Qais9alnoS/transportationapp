...
from src.config.dashboard_config import get_analytics_config
...
class AdvancedAnalyticsService:
    ...
    def analyze_geographic_hotspots(self) -> List[Dict[str, Any]]:
        ...
        if hotspots:
            # فلترة القيم غير الرقمية (خاصة في الاختبارات)
            valid_hotspots = [h for h in hotspots if isinstance(h.search_count, (int, float)) and isinstance(h.lat, (int, float, float)) and isinstance(h.lng, (int, float, float))]
            if not valid_hotspots:
                return []
            max_count = max(h.search_count for h in valid_hotspots)
            min_count = min(h.search_count for h in valid_hotspots)
    ...
    def generate_complaint_insights(self) -> List[str]:
        ...
        if avg_response_time > 86400:  # أكثر من 24 ساعة
            insights.append("وقت استجابة طويل - تحسين آلية معالجة الشكاوى")
    ...
    def get_service_usage_statistics(self) -> Dict[str, Any]:
        ...
        usage_stats = {
            "today": {
                ...
                "location_shares": self.db.query(LocationShare).filter(func.date(LocationShare.created_at) == today).count(),
            ...
            "week": {
                ...
                "location_shares": self.db.query(LocationShare).filter(LocationShare.created_at >= week_ago).count(),
            ...
            "month": {
                ...
                "location_shares": self.db.query(LocationShare).filter(LocationShare.created_at >= month_ago).count(),
